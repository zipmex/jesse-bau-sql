---- MTU 2021-11-17 -- active balance (Zipup/ Ziplock) -- active Trader -- active depositor/ withdrawer -- zipworld purchaser -- zlaunch staker
WITH base AS (
		SELECT 
			a.created_at 
	...
	WHERE (z_wallet_amount_usd >= 1 OR ziplock_amount_usd >= 1 OR wallet_usd_amount >= 1)
		...
		WHERE 
			...
			AND created_at >= '2022-05-01 00:00:00' 
...
--- deposit + withdrawl 
		...
		WHERE 
			COALESCE(d.month_, w.month_) >= '2022-05-01 00:00:00' 
...
SELECT
	created_at::date 
--	, signup_hostcountry
	, COUNT(DISTINCT mtu_3) mtu_count
