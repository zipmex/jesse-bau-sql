WITH base AS (
	...
	-- snapshot by end of month or yesterday
		AND ((a.created_at = DATE_TRUNC('month', a.created_at) + '1 month' - '1 day'::INTERVAL) OR (a.created_at = DATE_TRUNC('day', NOW()) - '2 day'::INTERVAL))
...
SELECT 
	created_at
	, signup_hostcountry
	, CASE		WHEN COALESCE( zmt_tradew_usd, 0) >= 1 AND COALESCE( non_zmt_tradew_usd, 0) >= 1 
				THEN ap_account_id END AS trade_mix_user_count
	, CASE		WHEN COALESCE( zmt_tradew_usd, 0) >= 1 AND COALESCE( non_zmt_tradew_usd, 0) < 1 
				THEN ap_account_id END AS trade_zmt_user_count
	, CASE		WHEN COALESCE( zmt_tradew_usd, 0) < 1 AND COALESCE( non_zmt_tradew_usd, 0) >= 1 
				THEN ap_account_id END AS trade_non_zmt_user_count
	, CASE		WHEN COALESCE( zmt_zipup_usd, 0) >= 1 AND COALESCE( non_zmt_zipup_usd, 0) >= 1 
	...
	, CASE		WHEN COALESCE( zmt_ziplock_usd, 0) >= 1 AND COALESCE( non_zmt_ziplock_usd, 0) >= 1 
...
SELECT 
	...
	, signup_hostcountry
	, COUNT(DISTINCT CASE 
				WHEN COALESCE( zmt_tradew_usd, 0) >= 1 AND COALESCE( non_zmt_tradew_usd, 0) >= 1 
				THEN ap_account_id END) AS trade_mix_user_count
	, COUNT(DISTINCT CASE 
				WHEN COALESCE( zmt_tradew_usd, 0) >= 1 AND COALESCE( non_zmt_tradew_usd, 0) < 1 
				THEN ap_account_id END) AS trade_zmt_user_count
...
--				THEN ap_account_id END) AS zmt_holder_total_count
FROM aum_zmt
GROUP BY
	1,2
;
