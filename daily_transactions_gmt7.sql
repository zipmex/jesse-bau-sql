...
DROP TABLE IF EXISTS warehouse.reportings_data.dm_user_transactions_dwt_daily;

--DELETE FROM warehouse.reportings_data.dm_user_transactions_dwt_daily WHERE created_at >= NOW()::DATE - '1 day'::INTERVAL;

CREATE TABLE IF NOT EXISTS warehouse.reportings_data.dm_user_transactions_dwt_daily 
(
...
CREATE INDEX IF NOT EXISTS idx_dm_user_transactions_dwt_daily ON warehouse.reportings_data.dm_user_transactions_dwt_daily 
(created_at, signup_hostcountry, ap_account_id, product_1_symbol);

ALTER TABLE warehouse.reportings_data.dm_user_transactions_dwt_daily REPLICA IDENTITY FULL;
...
(
    ...
    -- Pluang has a secondary account, data stored in zipmex_otc_public
        SELECT 
            DATE_TRUNC('day', q.created_at)::DATE created_at
...
--     exclude market maker
    ...
    FROM 
        all_trade a 
    WHERE 
       created_at >= '2020-11-01' -- DATE_TRUNC('day', NOW()::DATE) - '1 day'::INTERVAL
...
(
    ...
    ( 
        SELECT 
            date_trunc('day', d.updated_at + '7 hour'::INTERVAL) AS updated_at
            , d.ap_account_id 
    ...
    -- only take FullyProcessed Tickets
            ...
            AND CASE WHEN d.created_at + '7 hour'::INTERVAL < '2022-05-05' THEN d.ap_account_id NOT IN (SELECT DISTINCT ap_account_id FROM mappings.users_mapping)
    ...
    (
        SELECT 
            date_trunc('day', w.updated_at + '7 hour'::INTERVAL) AS updated_at
            , w.ap_account_id 
    ...
    -- only take FullyProcessed Tickets
            ...
            AND CASE WHEN w.created_at + '7 hour'::INTERVAL < '2022-05-05' THEN w.ap_account_id NOT IN (SELECT DISTINCT ap_account_id FROM mappings.users_mapping)
    ...
    WHERE 
       COALESCE(d.updated_at, w.updated_at)::DATE >= '2020-11-01' -- DATE_TRUNC('day', NOW()::DATE) - '1 day'::INTERVAL
...
(
    WITH temp_transactions AS (
        SELECT 
            COALESCE(d.created_at, t.created_at)::DATE created_at  
...
INSERT INTO warehouse.reportings_data.dm_user_transactions_dwt_daily (invitation_code, user_id, created_at , signup_hostcountry , ap_account_id
, product_1_symbol  , coin_buy_amount    , coin_net_buy_amount    , sum_coin_trade_amount	, usd_buy_amount    , usd_net_buy_amount     
